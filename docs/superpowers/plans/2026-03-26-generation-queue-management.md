...
import { db } from '../db'
import { generationJobs, generationBatches, projectScenes, projects } from '../db/schema'
import { eq, desc } from 'drizzle-orm'
import { synthesizePrompts } from '../services/prompt'
import { enqueueBatch, cancelPendingJobs, cancelBatch as cancelBatchService, getQueueStatus, getGlobalQueueStats, pauseQueue, resumeQueue, dismissError, getNextBatchOrder } from '../services/generation'
...
import { createServerFn } from '@tanstack/react-start'
import { db } from '../db'
import { generationJobs, generationBatches, generatedImages } from '../db/schema'
...
export const listQuickJobs = createServerFn({ method: 'GET' })
  .handler(async () => {
    ...
    return { jobs, batchTiming: globalStats.sessionTiming ? { startedAt: globalStats.sessionTiming.startedAt, totalImages: globalStats.totalImages, completedImages: globalStats.completedImages, avgImageDurationMs: globalStats.avgImageDurationMs } : null, queueStatus }
  })
```
...
export const listQueueBatches = createServerFn({ method: 'GET' }).handler(async () => {
  const batches = db
    ...
    .leftJoin(generationJobs, eq(generationJobs.batchId, generationBatches.id))
    .where(inArray(generationBatches.status, ['pending', 'running']))
  ...
  return (
    <div ref={setNodeRef} style={style} className="border border-border rounded-lg overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 bg-background">
        ...
        {/* Status indicator */}
        <span className={`size-2 rounded-full shrink-0 ${isRunning ? 'bg-primary animate-pulse' : 'bg-muted-foreground/30'}`} />

        {/* Label + expand toggle */}
        <button type="button" className="flex-1 min-w-0 text-left" onClick={onToggle}>
          <span className="text-sm font-medium truncate block">{batch.label}</span>
        </button>
        ...
        {/* Cancel */}
        <Button variant="ghost" size="icon-sm" onClick={onCancel} className="text-muted-foreground hover:text-destructive shrink-0">
      ...
      {isExpanded && jobs && (
        <div className="border-t border-border bg-muted/30">
          {jobs.map((job) => {
            const jobPct = (job.totalCount ?? 0) > 0 ? ((job.completedCount ?? 0) / (job.totalCount ?? 0)) * 100 : 0
            const jobRunning = job.status === 'running'
            const jobFailed = job.status === 'failed'
            const jobPending = job.status === 'pending'

            return (
              <div key={job.id} className="flex items-center gap-2 px-3 py-1.5 pl-9">
                <span className={`size-1.5 rounded-full shrink-0 ${
                  jobRunning ? 'bg-primary animate-pulse' : jobFailed ? 'bg-destructive' : 'bg-muted-foreground/30'
                }`} />
                <span className={`text-xs truncate flex-1 min-w-0 ${
                  jobFailed ? 'text-destructive' : jobRunning ? 'text-foreground' : 'text-muted-foreground'
                }`}>
                ...
                {jobPending && (
                  <Button variant="ghost" size="icon-sm" onClick={() => onCancelJob(job.id)} className="text-muted-foreground hover:text-destructive shrink-0 size-6">
...
function QueuePage() {
  ...
  const [summary, setSummary] = useState<Awaited<ReturnType<typeof fetchQueueSummary>> | null>(null)
  const [cancelTarget, setCancelTarget] = useState<{ type: 'batch'; id: number } | null>(null)
  ...
  async function handleBatchDragEnd(event: DragEndEvent) {
    ...
    await reorderQueueBatches({ data: { batchIds: reordered.map((b) => b.id) } })
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div>
          ...
          {activeBatches.length > 0 && (
            <p className="text-sm text-muted-foreground mt-0.5">
              ...
              {etaMs != null && ` · ${t('queue.eta' as any, { time: formatDuration(etaMs) })}`}
            </p>
          )}
        </div>
        <div className="flex items-center gap-1">
          {isProcessing && !isPaused && !isError && (
            <Button variant="secondary" size="sm" onClick={() => pauseGeneration()}>
              <HugeiconsIcon icon={PauseIcon} className="size-4" />
              {t('queue.pause' as any)}
            </Button>
          )}
          {(isPaused || isError) && (
            <Button variant="secondary" size="sm" onClick={() => resumeGeneration()}>
      ...
      {activeBatches.length === 0 ? (
        <div className="text-center py-16 space-y-3">
          ...
          <p className="text-sm text-muted-foreground/70">{t('queue.emptyDescription' as any)}</p>
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleBatchDragEnd}>
          <SortableContext items={activeBatches.map((b) => b.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-2">
              {activeBatches.map((batch) => (
                <SortableBatchItem
                  ...
                  onCancel={() => setCancelTarget({ type: 'batch', id: batch.id })}
      ...
      {recentBatches.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-muted-foreground mb-2">{t('queue.recentHistory' as any)}</h2>
          <div className="space-y-1">
            {recentBatches.map((batch) => {
              const isCompleted = batch.status === 'completed'
              const isFailed = batch.status === 'failed'
              return (
                <div key={batch.id} className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border">
                  <span className={`size-2 rounded-full shrink-0 ${
                    isCompleted ? 'bg-green-500' : isFailed ? 'bg-destructive' : 'bg-muted-foreground/30'
                  }`} />
                  <span className="text-sm truncate flex-1 min-w-0">{batch.label}</span>
                  <span className="text-xs tabular-nums text-muted-foreground shrink-0">
                    {batch.completedImages ?? 0}/{batch.totalImages ?? 0}
                  </span>
                  {isFailed && (
                    <Button variant="ghost" size="icon-sm" onClick={() => handleRetryBatch(batch.id)} className="text-muted-foreground hover:text-foreground shrink-0">
...
export function QueueStatusWidget() {
  ...
  const isPaused = summary.queueStopped === 'paused'
  const isError = summary.queueStopped === 'error'
  const barColor = isError ? 'bg-destructive' : isPaused ? 'bg-amber-500' : 'bg-primary'
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-xs text-muted-foreground tabular-nums">
        ...
        {isPaused && <span className="text-amber-500 font-medium">{t('queue.paused' as any)}</span>}
        {isError && <span className="text-destructive font-medium">{t('queue.error' as any)}</span>}
      </div>
    </Link>
  )
}
