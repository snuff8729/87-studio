...
import { db } from '../db'
import { generationBatches, generationJobs, projects, projectScenes } from '../db/schema'
...
export const listQueueBatches = createServerFn({ method: 'GET' }).handler(async () => {
  const batches = db
    .select({
...
export const listRecentBatches = createServerFn({ method: 'GET' }).handler(async () => {
  const batches = db
    .select({
      id: generationBatches.id,
      projectId: generationBatches.projectId,
    ...
    .from(generationBatches)
    .where(inArray(generationBatches.status, ['completed', 'failed', 'cancelled']))
...
export const reorderQueueBatches = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    log.info('reorderQueueBatches', 'Reordering batches', { batchIds: data.batchIds })
...
export const reorderBatchJobsFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    log.info('reorderBatchJobs', 'Reordering jobs in batch', { batchId: data.batchId, jobIds: data.jobIds })
...
export const retryBatch = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: batchId }) => {
    ...
    log.info('retryBatch', 'Retrying failed batch', { originalBatchId: batchId, newBatchId: newBatch.id, jobCount: failedJobs.length })
    enqueueBatch(newBatch.id)
    return newBatch
  })

/** Combined queue status + global stats */
export const fetchQueueSummary = createServerFn({ method: 'GET' }).handler(async () => {
